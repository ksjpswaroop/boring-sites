import XCTest
import WordGameCore
@testable import LexJoltApp

final class LexJoltAppTests: XCTestCase {
    ...
    func testV1CatalogContainsExactlyTheFiveLockedGames() {
        XCTAssertEqual(LexJoltCatalog.v1Games, [
            .wordUnscrambler,
            .anagramRush,
            .dailyScramble,
            .spellingBee,
        ...
        XCTAssertEqual(LexJoltCatalog.v1Games.map(\.title), [
            "Word Unscrambler",
            "Anagram Rush",
            "Daily Jolt",
            "Spelling Bee",
    ...
    func testV1PolicyHasNoAdsLoginBackendOrPaymentsAndSupportsOfflinePlay() {
        XCTAssertFalse(LexJoltPolicy.hasAds)
        XCTAssertFalse(LexJoltPolicy.requiresLogin)
        XCTAssertFalse(LexJoltPolicy.usesBackend)
        XCTAssertFalse(LexJoltPolicy.hasPayments)
        XCTAssertTrue(LexJoltPolicy.supportsOfflinePlay)
    }
    ...
    func testViewModelSchedulesDailyReminderThroughInjectedScheduler() async throws {
        let scheduler = RecordingReminderScheduler()
        let viewModel = LexJoltViewModel(reminderScheduler: scheduler)
    ...
    func testViewModelExposesTodayScreenData() {
        let viewModel = LexJoltViewModel(progress: LexJoltProgress(completedSessions: 2, currentStreak: 1))

        XCTAssertEqual(viewModel.games, LexJoltCatalog.v1Games)
    ...
    func testViewModelRecordsSanitizedAnalyticsForCompletionAndShare() throws {
        ...
        let viewModel = LexJoltViewModel(analyticsRecorder: recorder)
    ...
    func testPhase3DeviceCatalogDefinesAdaptedAppleEcosystemRoles() {
        XCTAssertEqual(LexJoltEcosystem.phase3Experiences.map(\.role), [
            .iPad,
            .mac,
            .watch,
            .tv,
        ...
        XCTAssertEqual(LexJoltEcosystem.experience(for: .iPad).primaryPattern, .splitViewDashboard)
        XCTAssertEqual(LexJoltEcosystem.experience(for: .mac).primaryPattern, .keyboardDashboard)
        XCTAssertEqual(LexJoltEcosystem.experience(for: .watch).primaryPattern, .companionGlance)
        XCTAssertEqual(LexJoltEcosystem.experience(for: .tv).primaryPattern, .partyMode)
        XCTAssertEqual(LexJoltEcosystem.experience(for: .vision).primaryPattern, .spatialBoard)
    }
    ...
    func testWatchCompanionShowsStreakReminderAndDailyCompletionStatus() {
        let incomplete = WatchCompanionModel(
            progress: LexJoltProgress(completedSessions: 5, currentStreak: 3),
            isDailyComplete: false
        )
        let complete = WatchCompanionModel(
            progress: LexJoltProgress(completedSessions: 6, currentStreak: 4),
    ...
    func testAnalyticsEventsArePrivacyPreservingAndRejectRawInputs() {
        ...
        XCTAssertEqual(sanitized.properties[.game], "Daily Jolt")
    ...
    func testTestFlightPlanDocumentsRequiredLaunchInputs() {
        ...
        XCTAssertEqual(plan.bundleIdentifier, "com.lexjolt.app")
